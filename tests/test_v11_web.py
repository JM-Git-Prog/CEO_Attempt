...
def test_v14_is_default_and_invalid_versions_are_rejected(client):
    page = client.get("/")
    assert page.status_code == 200
    assert "window.APP_VERSION=14" in page.text
    assert "Real 3D meshes" in page.text
    assert 'href="/?v=13"' in page.text and 'href="/?v=14"' in page.text

    for value in ("nope", "3.0", "02", "2", "15"):
