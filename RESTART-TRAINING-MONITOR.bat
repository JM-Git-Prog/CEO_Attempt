...
echo   3. Makes sure the bench-loop harvester is running (it feeds the flywheel
echo      its new organic training rows). If it's not running, starts it.
echo   4. Closes and reopens the Training Monitor so you're watching it fresh.
echo.

echo [1/5] Checking whether training is actively running right now...
...
echo [2/5] Checking the flywheel loop...
...
echo [3/5] Checking the bench-loop harvester...
set BENCHLOOP_RUNNING=0
tasklist /FI "WINDOWTITLE eq bench-loop" 2>NUL | find /I "cmd.exe" >NUL
if not errorlevel 1 set BENCHLOOP_RUNNING=1

if "%BENCHLOOP_RUNNING%"=="0" echo       Not running - starting it now.
if "%BENCHLOOP_RUNNING%"=="0" start "bench-loop" /min cmd /c "cd /d "C:\Users\JohnM\My Applications\Kiro\CEO_Kiro\CEO_Attempt" && python bench\bench_loop.py >> bench\bench-loop-console.txt 2>&1"
if "%BENCHLOOP_RUNNING%"=="1" echo       Already running - left alone.

echo [4/5] Closing any existing Training Monitor window...
taskkill /FI "WINDOWTITLE eq Training Monitor - The Living Room" /F >NUL 2>&1

echo [5/5] Opening a fresh Training Monitor...
