...
def _canon_prompt(plan: dict) -> str:
    v, pal = plan.get("vibe", {}), plan.get("palette", {})
    objs = ", ".join(o["name"] for o in plan.get("objects", []) if o.get("category") != "clutter")
    return (f"Interior photograph of a {v.get('era','')} {v.get('style','room')}, {v.get('condition','')}, "
            f"{pal.get('mood','calm')} mood, a {plan.get('width_m')}x{plan.get('depth_m')} meter room, "
            f"containing {objs}, wooden trim, warm practical lighting, natural window light, "
...
async def make_canon(sid: str, roll: int = 0):
    ...
    dest = sdir / "canon.png"
    if dest.exists() and int(roll) == 0:
        return {"ok": True, "cached": True}
    if dest.exists():                                   # re-dream: every prior dream is kept
        dest.rename(sdir / f"canon.re-dream-{int(time.time())}.bak.png")
    plan = json.loads(plan_path.read_text(encoding="utf-8"))
    prompt = _canon_prompt(plan)
...
async def reconcile(sid: str):
    ...
    try:
        async with httpx.AsyncClient(timeout=20.0) as cl:
            ...
            for n, (i, o) in enumerate(targets):
                prog.write_text(json.dumps({"done": n, "total": len(targets), "current": o["name"]}))
                sub = await cl.post(f"{COMFY_MAIN}/prompt", json={"prompt": _sam3_graph(canon_name, o["name"])})
                pid = sub.json()["prompt_id"]
                cut = None
                for _ in range(30):
    ...
    verdict = "green" if coverage >= 0.6 and order_ok and len(raw) >= 3 else \
